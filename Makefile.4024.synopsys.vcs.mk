...
# make sure use gcc 4.4
#vcsPara1:= -debug_access+all -full64 
#vcsPara1:= -debug_access+all -full64 -cpp no-pie-g++
#vcsPara1:= -lca -debug_access+all -full64 -cpp no-pie-g++
#vcsPara1:= -lca -debug_access+all -full64
#vcsPara1:= -lca -debug_access+all
#vcsPara1:= -debug_access+all
#vcsPara1:= -debug_access+all -LDFLAGS -Wl,--no-as-needed -LDFLAGS -fPIC
#vcsPara1:= -lca -debug_access+all -LDFLAGS -Wl,--no-as-needed -LDFLAGS -fPIC
#vcsPara1:= -lca -debug_access+all -LDFLAGS -Wl,--no-as-needed -LDFLAGS -fPIC -full64
#vcsPara1:= -lca -debug_access+all
#vcsPara1:= -lca -debug_access+all -full64 -LDFLAGS -Wl,--no-as-needed
vcsPara1:= -lca -debug_access+all -full64 
...
ws2:=all_vcs_with_Verdi_FSDB
$(ws2):=cvs bs2 ts2
ws2:$($(ws2))

synopsysVCS_OpList:=cvs bs1 bs2 ts1 ts2 ws1 ws2

define VCShelp

/e/eda2321/VCS_2016.06/bin/vcs -help

Warning-[LINX_KRNL] Unsupported Linux kernel
  Linux kernel '4.13.0-45-generic' is not supported.
  Supported versions are 2.4* or 2.6*.

vcs script version : L-2016.06
machine name = jg1 
machine type = linux 
machine os = Linux 4.13.0-45-generic  
The FLEXlm host ID of this machine is "cc2f710703be 309c23309fd9"
Compiler version = VCS-MX L-2016.06
...
-cm_count
   ...
   In Line Coverage, not just whether a line was executed, but how many 
   times.
...
-cm_fsmresetfiltser <filename>
   Filters out transitions in assignment statements controlled by
   if statements where the conditional expression (following the
   keyword if) is a signal you specify in the file.
...
-cm_pp [gui]|[batch]
   Tells VCS to start cmView, it starts cmView in batch mode to write reports 
   by default. 

   gui
     Specifies starting the cmView graphical user interface instead of
     writing reports.

   batch
     Specifies starting cmView in batch mode to write reports. This
     keyword argument is not necessary because batch mode is the default
     condition.

   You enter cmView command line options to the right of this option and its 
   argument.

-cm_resetfilter
   You can filter out of FSM coverage transitions in assignments 
   controlled controlled by if statements where the conditional
   expression (following the if keyword) is a signal you specify 
   in the file. This filtering out can be on the specified signal
   in any module or the module you specify in the file. You can
   also specify the FSM and whether the signal is true or false.
...
-debug_access
   Enables post-process debug.  Use'-debug_access+classdbg' for testbench debug,
   and '-debug_access+all' for all debug capabilities.  Refer the VCS user
   guide for more granular options for debug control under the switch
   '-debug_access' and refer to '-debug_region' for region control."

-debug_pp
   Enables dumping to VPD and use of UCLI commands and DVE.
...
-ova_cov_hier <filename>
   Limits functional coverage to the module instances listed in the 
   specified file.
...
+pli_unprotected
   Enables PLI and CLI access to the modules in the protected source 
   file being created (PLI and CLI access is normally disabled for 
   protected modules).
...
+vcs+vcdpluson
   ...
   switch enables dumping for the entire design. You would however need 
   to use a debug switch (example -debug_pp) to dump the data.
...
-k <filename> | off
   ...
   into which VCS writes the CLI interactive commands that you enter 
...
-ova_cov_db <filename>
   Specifies the path name of the initial coverage file. The initial coverage 
   file is needed to set up the database. By default, an empty coverage file 
   is loaded from simv.vdb/snps/fcov/default.db.
...
+cliecho
   Specifies that VCS displays CLI commands in a file that you specify
   with the -i option as VCS executes these commands. UNIX only.
...
-PP
   Enables you to enter in your Verilog source code system tasks like 
   $vcdpluson to create a VPD file during simulation. This option 
